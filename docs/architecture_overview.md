...
Этот документ нужен не для запуска, а для быстрого понимания того, **как именно устроен сервис** и где в нем работает LLM, а где обычный Python-код
...
- **Google ADK agent**
- **один tool boundary**
- **детерминированное Python-ядро**

То есть агент отвечает за диалог и сбор входных параметров, а подбор моделей, расчет стоимости и сбор итогового отчета делает код
...
Это осознанное архитектурное решение, а не недоделка
